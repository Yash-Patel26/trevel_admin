...
try {
const { order_id, refund_amount, currency = 'INR', refund_reference, reason } = req.body || {};
if (!order_id || !refund_amount) {
return res.status(400).json({
success: false,
error: 'Missing required fields',
message: 'order_id and refund_amount are required'
});
}
const refundAmountPaise = toPaise(refund_amount);
