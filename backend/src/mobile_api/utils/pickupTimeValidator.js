 ...
 */
function validatePickupTime(pickupDate, pickupTime, minHoursAhead = -24) {
  ...
  // Add 5.5 hours to current UTC time to get IST time values in a "UTC" date object
  // formatted as if the system was in IST
  ...
  if (totalMinutesDiff < requiredMinutes) {
    ...
    return {
      valid: false,
      error: `Pickup time must be at least ${minHoursAhead} hours from now. Current time: ${currentTimeStr} (IST), Minimum allowed: ${minAllowedTimeStr} (IST), Selected: ${pickupTimeStr} (IST). Please ensure you are booking for a future time.`,
